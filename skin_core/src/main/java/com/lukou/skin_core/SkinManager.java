...
public class SkinManager extends Observable {
     ...
     * @param application
     */
    public static void init(Application application, List<String> skinActivityNames, String skinPrefix) {
        if (instance == null) {
            synchronized (SkinManager.class) {
                if (instance == null) {
                    instance = new SkinManager(application, skinActivityNames, skinPrefix);
                }
            }
        }
    }

    private SkinManager(Application application, List<String> skinActivityNames, String skinPrefix) {
        mContext = application;
        //共享首选项 用于记录当前使用的皮肤
        SkinPreference.init(application);
        //资源管理类 用于从 app/皮肤 中加载资源
        SkinResources.init(application, skinPrefix);
        //注册Activity生命周期
...
//        loadSkin(SkinPreference.getInstance().getSkin());
        mActivityNames = skinActivityNames;
    }
     ...
     */
    public synchronized void loadSkin(String skinPath) {
        // 如果皮肤路径与当前一样， 不做任何处理
        if (mIsInit && skinPath.equals(SkinPreference.getInstance().getSkin())) {
            return;
        }
